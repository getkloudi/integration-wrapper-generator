...
exports.getFunctionWithParams = function getFunctionWithParams(contents) {
    ...
    while ((v = regex.functionWithParamsRegex.exec(contents)) !== null) {
        ...
        let w;
        while ((w = regex.functionNameFromFunctionWithParamRegex.exec(v[0])) !== null) {
            functionName = w[0];
        ...
        while ((w = regex.functionParamsFromFunctionWithParamRegex.exec(v[0])) !== null) {
            functionParams = w[0];
        ...
        for (let i = 0; i < functionParams.length; i++) {
            newFunctionParamsToReplace = newFunctionParamsToReplace.replace(`${functionParams[i]}`, `incomingOptions.${functionParams[i]}`)
        }
...
exports.generateCodeFile = function generateCodeFile(codeBlocks, functionWithParams) {
    ...
    for (let i = 0; i < codeBlocks.length; i++) {
        ...
        codeBlock = codeBlock.replace('\'YOUR ACCESS TOKEN\'', 'incomingOptions.accessToken');
        ...
        for (let j = 0; j < functionWithParams[i].functionParams.length; j++) {
            ...
            codeBlock = codeBlock.substr(0, indexOfVariable - 2) + "\n/*" + codeBlock.substr(indexOfVariable);

            // Add ending comment
            codeBlock = codeBlock.substr(0, semiColonIndex + indexOfVariable + 1) + ";*/" + codeBlock.substr(semiColonIndex + indexOfVariable + 2);

            // incomingOptions change
            codeBlock = codeBlock.replace(functionWithParams[i].functionParamToReplace, functionWithParams[i].newFunctionParamsToReplace);

            // This can be added on need basis
            // codeBlock = codeBlock.replace('console.error(error);', 'return error;');

            codeBlock = codeBlock.replace(`console.log('API called successfully.');`, `return;`)

            codeBlock = codeBlock.replace(`console.log('API called successfully. Returned data: ' + data);`, 'return data;');
        }
