...
class SentryService {
    ...
    async registerWebhooks(incomingOptions) {
        ...
        if (config[0].value && config[0].value.split('\n').indexOf(webhookURL) < 0) {
            try {
        ...
        try {
            ...
            if (alertNames.indexOf(`Alert Rules for Kloudi ${nconf.get('ENV')}`) >= 0) return 'Ok';
    ...
    async syncIntegrationEntities(integration, incomingOptions) {
        ...
        try {
            const res = await Axios.default.post(
                taskUri,
                {
                    pepper_task: ['task.pepper.SYNC_LEGACY_SENTRY_ISSUES', 'task.pepper.SYNC_SENTRY_TEAM_MEMBERS'],
                    ...
                    third_party_organization_id: incomingOptions.thirdPartyProject.organizationId,
                },
                {
                    headers: {
    ...
    async getMembers(options) {
        ...
        try {
            let count = 0;
            while (parsedHeader && parsedHeader.next && parsedHeader.next.results == 'true') {
                const res = await Axios.default.get(parsedHeader.next.url, {
                    headers: {
                        Authorization: `Bearer ${options.accessToken}`,
                    },
    ...
    async updateIssues(options) {
        if (!options.body.organization_slug || !options.body.project_slug) {
            ErrorHelper.sendErrorToThirdPartyTool(new Error('No third party projects present'));
            return { data: 'Error' };
        }

        let params = {};
        const optionKeys = Object.keys(options.body);
        if (optionKeys.includes('hasSeen')) params = { hasSeen: options.body.hasSeen };
        if (optionKeys.includes('status')) params = { hasSeen: true, status: options.body.status };

        if (Object.keys(params) <= 0) return { data: 'Nothing to update' };
        try {
            const data = await Axios.default.put(
                `${this.apiEndpoint}/projects/${options.body.organization_slug}/${options.body.project_slug}/issues/?${options.body.id}`,
                params,
                {
    ...
    async updateIssue(options) {
        if (!options.integration.thirdPartyProjects || options.integration.thirdPartyProjects.length == 0) {
        ...
        if (optionKeys.includes('status')) params = { hasSeen: true, status: options.status };

        if (Object.keys(params) <= 0) return { data: 'Nothing to update' };
        try {
            const data = await Axios.default.put(`${this.apiEndpoint}/issues/${options.id}/`, params, {
                headers: {
            ...
            ErrorHelper.sendErrorToThirdPartyTool(error.response.data || error.response || error);
            return { data: error.response.data };
        }
    }
    ...
    async getIssueFromID(options) {
        if (!options.query.id) throw Error('options.id is missing from the request body');
        const res = await Axios.default.get(`${this.apiEndpoint}/issues/${options.query.id}/?statsPeriod=`, {
            headers: {
                Authorization: `Bearer ${options.accessToken}`,
            },
        });
