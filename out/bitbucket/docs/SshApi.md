...
> SshAccountKey usersUsernameSshKeysGet(username, keyId)
...
let username = "username_example"; // String | The account's username or UUID.
let keyId = "keyId_example"; // String | The SSH key's UUID value.
apiInstance.usersUsernameSshKeysGet(username, keyId, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
> PaginatedSshUserKeys usersUsernameSshKeysGet_0(username)
...
apiInstance.usersUsernameSshKeysGet_0(username, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
...
> SshAccountKey usersUsernameSshKeysPost(username, opts)
