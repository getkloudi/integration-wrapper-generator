"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports["default"] = void 0;

var _ApiClient = _interopRequireDefault(require("../ApiClient"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { "default": obj }; }

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

function _defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } }

function _createClass(Constructor, protoProps, staticProps) { if (protoProps) _defineProperties(Constructor.prototype, protoProps); if (staticProps) _defineProperties(Constructor, staticProps); return Constructor; }

/**
 * The AttachNetworkInterfaceRequest model module.
 * @module model/AttachNetworkInterfaceRequest
 * @version 1.0.0
 */
var AttachNetworkInterfaceRequest =
/*#__PURE__*/
function () {
  /**
   * Constructs a new <code>AttachNetworkInterfaceRequest</code>.
   * Contains the parameters for AttachNetworkInterface.
   * @alias module:model/AttachNetworkInterfaceRequest
   * @param deviceIndex {Number} 
   * @param instanceId {String} 
   * @param networkInterfaceId {String} 
   */
  function AttachNetworkInterfaceRequest(deviceIndex, instanceId, networkInterfaceId) {
    _classCallCheck(this, AttachNetworkInterfaceRequest);

    AttachNetworkInterfaceRequest.initialize(this, deviceIndex, instanceId, networkInterfaceId);
  }
  /**
   * Initializes the fields of this object.
   * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
   * Only for internal use.
   */


  _createClass(AttachNetworkInterfaceRequest, null, [{
    key: "initialize",
    value: function initialize(obj, deviceIndex, instanceId, networkInterfaceId) {
      obj['DeviceIndex'] = deviceIndex;
      obj['InstanceId'] = instanceId;
      obj['NetworkInterfaceId'] = networkInterfaceId;
    }
    /**
     * Constructs a <code>AttachNetworkInterfaceRequest</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/AttachNetworkInterfaceRequest} obj Optional instance to populate.
     * @return {module:model/AttachNetworkInterfaceRequest} The populated <code>AttachNetworkInterfaceRequest</code> instance.
     */

  }, {
    key: "constructFromObject",
    value: function constructFromObject(data, obj) {
      if (data) {
        obj = obj || new AttachNetworkInterfaceRequest();

        if (data.hasOwnProperty('DeviceIndex')) {
          obj['DeviceIndex'] = _ApiClient["default"].convertToType(data['DeviceIndex'], 'Number');
        }

        if (data.hasOwnProperty('DryRun')) {
          obj['DryRun'] = _ApiClient["default"].convertToType(data['DryRun'], 'Boolean');
        }

        if (data.hasOwnProperty('InstanceId')) {
          obj['InstanceId'] = _ApiClient["default"].convertToType(data['InstanceId'], 'String');
        }

        if (data.hasOwnProperty('NetworkInterfaceId')) {
          obj['NetworkInterfaceId'] = _ApiClient["default"].convertToType(data['NetworkInterfaceId'], 'String');
        }
      }

      return obj;
    }
  }]);

  return AttachNetworkInterfaceRequest;
}();
/**
 * @member {Number} DeviceIndex
 */


AttachNetworkInterfaceRequest.prototype['DeviceIndex'] = undefined;
/**
 * @member {Boolean} DryRun
 */

AttachNetworkInterfaceRequest.prototype['DryRun'] = undefined;
/**
 * @member {String} InstanceId
 */

AttachNetworkInterfaceRequest.prototype['InstanceId'] = undefined;
/**
 * @member {String} NetworkInterfaceId
 */

AttachNetworkInterfaceRequest.prototype['NetworkInterfaceId'] = undefined;
var _default = AttachNetworkInterfaceRequest;
exports["default"] = _default;